...
data class MultiObjectSegmentationResult(
    ...
    val combinedMaskBase64: String?, // Base64 encoded mask
...
class DeepLabV3XceptionSegmenter(
    ...
    suspend fun segmentMultipleObjects(bitmap: Bitmap, targetClassIndices: List<Int>): MultiObjectSegmentationResult = withContext(Dispatchers.Default) {
        ...
        if (interp == null) {
            ...
            return@withContext MultiObjectSegmentationResult(emptyMap(), null, null, 0L, 0, targetClassIndices.size)
        }
        ...
        val finalCombinedMask = if (successCount > 0) combinedMask else null

        // Base64'e çevir
        val combinedMaskBase64 = finalCombinedMask?.let { bitmap ->
            try {
        ...
        Log.d(TAG, "Çoklu segmentasyon tamamlandı: $successCount/${targetClassIndices.size} başarılı, süre: ${totalProcessingTime}ms")

        return@withContext MultiObjectSegmentationResult(
            individualResults = individualResults,
            combinedMaskBitmap = finalCombinedMask,
            combinedMaskBase64 = combinedMaskBase64,
            totalProcessingTimeMs = totalProcessingTime,
            successCount = successCount,
            totalCount = targetClassIndices.size
        )
    }
