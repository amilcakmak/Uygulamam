// Dosya: app/src/main/java/com/rootcrack/aigarage/navigation/Navigations.kt
...
import com.rootcrack.aigarage.screens.WebViewerScreen
import com.rootcrack.aigarage.viewmodel.CameraViewModel // Açıklama: Import path düzeltildi
...
sealed class Screen(val route: String) {
    ...
    object EditPhoto : Screen("ai_processing_screen/{${NavArgs.IMAGE_URI}}/{${NavArgs.INSTRUCTION}}/{${NavArgs.MASK}}/{${NavArgs.EDIT_TYPE}}")
    ...
    object WebViewer : Screen("web_viewer_screen")
...
// Açıklama: Modern bottom bar görünürlük yönetimi
private val bottomBarVisibleScreens = setOf(
    Screen.Home.route,
    Screen.Gallery.route,
    Screen.Settings.route,
    Screen.WebViewer.route
)

private fun shouldShowBottomBar(currentRoute: String?): Boolean {
    return currentRoute in bottomBarVisibleScreens &&
           !currentRoute.isDetailOrPreviewScreen()
}

private fun String?.isDetailOrPreviewScreen(): Boolean {
    if (this == null) return false
    return startsWith(Screen.PhotoDetail.route.substringBefore("/{")) ||
           startsWith(Screen.PhotoPreview.route.substringBefore("/{")) ||
           startsWith(Screen.EditPhoto.route.substringBefore("/{")) ||
           startsWith(Screen.AutoMaskPreview.route.substringBefore("/{")) ||
           startsWith(Screen.WebViewer.route.substringBefore("/{"))
}
...
fun AppNavigation(
    ...
    val showGlobalBottomBar = shouldShowBottomBar(currentRoute) && currentRoute != Screen.Camera.route
    ...
    Scaffold(
        ...
        NavHost(
            ...
            // Açıklama: Navigation animasyonları eklendi
            enterTransition = {
                fadeIn(
            ...
            composable(
                ...
                PhotoPreviewScreen(
                    navController = navController,
                    imageUri = imageUriString?.let { Uri.decode(it) }
                )
            }
            ...
            composable(
                ...
                PhotoPreviewScreen(
                    navController = navController,
                    imageUri = encodedImageUri?.let { Uri.decode(it) }
                )
            }
            ...
            composable(
                ...
                AIProcessingScreen(
                    ...
                    editType = editType,
                                         onProcessPhoto = { originalImageUri, maskBase64, instruction, onSuccess ->
                         coroutineScope.launch {
                             ...
                             if (processedUri != null) {
                                 // Açıklama: İşlenen resmi AIProcessingScreen'de göster
                                 onSuccess(processedUri.toString())
        ...
        composable(Screen.Gallery.route) {
            ...
            composable(
                ...
                arguments = listOf(
                    ...
                    navArgument(NavArgs.MODEL_PATH) { type = NavType.StringType }
                )
            ) { backStackEntry ->
                val imageUriString = backStackEntry.arguments?.getString(NavArgs.IMAGE_URI)
                val objectToMask = backStackEntry.arguments?.getString(NavArgs.OBJECT_TO_MASK)
                val modelPath = backStackEntry.arguments?.getString(NavArgs.MODEL_PATH)
                ...
                AutoMaskPreviewScreen(
                    ...
                    modelPath = Uri.decode(modelPath)
                )
            }
            
            composable(Screen.WebViewer.route) {
                WebViewerScreen(navController = navController)
            }
